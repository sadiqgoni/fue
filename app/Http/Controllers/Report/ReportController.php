...
//            return    $document->stream($file_name."pdf");
            return view('reports.payrollJs', compact('payrolls', 'summaries', 'name_search', 'name'));
        } else {
...
//         $log->save();
// //        {{dd()}}
//         return view('reports.payslipJs',compact('paySlips',));
//         // start
...
//                $repo->save();
            ...
            return view('reports.deduction_scheduleJs', ['reports' => $reports_data, 'date_from' => $date_from]);
...
//        }
    ...
    public function pfa(Request $request)
    {
        ...
        $reports = SalaryHistory::join('p_f_a_s', 'salary_histories.pfa_name', 'p_f_a_s.id')
            ...
            ->when($request->group_by, function ($query) {
                return $query->where('salary_histories.pfa_name', request()->group_by);
            })
    ...
    public function employer_pension(Request $request)
    {
        ...
        $reports = SalaryHistory::when($request->salary_structure, function ($query, ) {
            ...
            ->when($request->group_by, function ($query) {
                return $query->where('pfa_name', request()->group_by);
            })
