...
class AnnualSalaryIncrement extends Component
{
    ...
    public $employee_type,
        ...
        $grade_level_to,
        $status;
    public $number_of_increment,$increment_date,$count,$ids;
    use LivewireAlert;
    protected $rules=[
    ...
    public function confirm()
    {
        ...
        if ($exists){
            $this->alert('question', 'Annual increment has been performed for the selected month, do you want to overwrite', [
               ...
               if (!\App\Models\AnnualSalaryIncrement::where('employee_id', $employee->id)->whereDate('month_year', Carbon::parse($this->increment_date)->format('Y-m-d'))->exists())
               {
                   ...
                   if ($salary_structure != null) {
                       if ($employee->step < $salary_structure->no_of_grade_steps) {
                           ...
                           $employee->step = $grade_step;
                           $employee->save();
                           $incrementObjToDelete = \App\Models\AnnualSalaryIncrement::where('increment_year',Carbon::parse($this->increment_date));
                           $incrementObjToDelete->delete();
                           $incrementObj =new \App\Models\AnnualSalaryIncrement();
